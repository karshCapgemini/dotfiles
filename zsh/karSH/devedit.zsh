...
#
# vi, short for vim interactive, extends the script further by using fzf and batcat to find all files
...
vi() {
	...
	# use fzf to select files, displaying with batcat
	local FILES_OPEN=$(
		if [ "$term_width" -lt 140 ]; then
			...
			fzf -m \
				...
				--preview='echo -e "$(basename {})" && batcat --color=always --number {}'
		else
			echo "$FILES" |
			sort |
			fzf -m \
